...
/*************************************************************************************************************
 *  Force measurement test-rig firmware. Measures Actuator position, Flow rate and Force.
 *  Drives 1 linear-actuator pinch-valve. Needs 1 Force sensor to be connected. If ENABLE_SENSORS then 
 *  also needs 1 flow sensor to be connected.
 * 
 ************************************************************************************************************/

// Uncomment below to Enable sensors
  ...
  if(flag_write_data)
  {
      ...
      if (ret_sfm3000 == 0)
      {
        if(USE_SERIAL_MONITOR)
        {
          ...
          SerialUSB.println(force_value);
        
        }
        else
        {
          // Send data (1 steppers, 1 force sensor, 1 flow sensor)      
        ...
        if(USE_SERIAL_MONITOR)
          {
              ...
              SerialUSB.print(",");
              SerialUSB.print("Force:");
              SerialUSB.print(force_value);
            
          }
          else
          {
            // Send data (1 steppers, 1 force sensor)          
    ...
    // close the valve
    if (flag_close_valve2_in_progress && ret_sfm3000 == 0)
    {
      if(mFlow >= 0.2)
      {
      ...
      {
        if(flag_valve2_flow_detected && mFlow < 0.2)
        {
          Y_commanded_target_position = (stepper_Y.currentPosition()+1);
          stepper_Y.runToNewPosition(Y_commanded_target_position);
          stepper_Y.setCurrentPosition(0);
