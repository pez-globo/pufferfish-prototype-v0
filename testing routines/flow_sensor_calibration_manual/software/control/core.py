...
class Waveforms(QObject):
    ...
    signal_dP_SDP31 = Signal(float,float)
    signal_dP_SDP810 = Signal(float,float)
    signal_dP_SM9336 = Signal(float,float)
        ...
        if SIMULATION:
            ...
            if readout is not None:
                ...
                for i in range(MCU.TIMEPOINT_PER_UPDATE):
                    ...
                    self.dP_SDP31 = utils.unsigned_to_signed(readout[i*MCU.RECORD_LENGTH_BYTE+6:i*MCU.RECORD_LENGTH_BYTE+8],2)/(65536/2)*MCU.dP_FS
                    self.dP_SDP810 = utils.unsigned_to_signed(readout[i*MCU.RECORD_LENGTH_BYTE+8:i*MCU.RECORD_LENGTH_BYTE+10],2)/(65536/2)*MCU.dP_FS
                    self.dP_SM9336 = utils.unsigned_to_signed(readout[i*MCU.RECORD_LENGTH_BYTE+10:i*MCU.RECORD_LENGTH_BYTE+12],2)/(65536/2)*MCU.dP_FS

                    record_from_MCU = (
                        str(self.time_ticks) + '\t' + str(self.flow_1) + '\t' + "{:.2f}".format(self.dP_SDP31) + '\t' + "{:.2f}".format(self.dP_SDP810) + '\t' + "{:.2f}".format(self.dP_SM9336) )
                ...
                if self.counter_display>=1:
                    ...
                    self.signal_Flow1.emit(self.time,self.flow_1)
                    # self.signal_Paw.emit(self.time,self.pressure_aw_cmH2O)
                    self.signal_dP_SDP31.emit(self.time,self.dP_SDP31)
                    self.signal_dP_SDP810.emit(self.time,self.dP_SDP810)
                    self.signal_dP_SM9336.emit(self.time,self.dP_SM9336)
