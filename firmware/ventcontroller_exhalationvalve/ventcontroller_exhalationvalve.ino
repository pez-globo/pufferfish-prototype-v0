...
static const int pin_valve2 = 41;
static const int pin_valve2_gnd = 40;
static const int pin_valve2_release = 31;
...
void setup() {
  ...
  pinMode(pin_valve2_gnd, OUTPUT);
  digitalWrite(pin_valve2_gnd, LOW);
  
  pinMode(pin_valve2, OUTPUT);
  digitalWrite(pin_valve2, HIGH);

  pinMode(pin_valve2_release, OUTPUT);
  digitalWrite(pin_valve2_release, LOW);
...
void set_valve2_state(int state)
{
  if(state>0)
  {
    ...
    digitalWrite(pin_valve2_release, HIGH);
  }
  else
  {
    digitalWrite(pin_valve2_release, LOW);
